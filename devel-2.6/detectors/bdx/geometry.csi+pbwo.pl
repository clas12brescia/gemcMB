...
sub make_mutest_minibdx
{
    ...
    my $ov_ir=17.7;
    $par1 = $ov_ir/2; #InRad
    $par2 = ($ov_ir+$ov_side_tk)/2; #InRad+thick
   ...
   print_det(\%configuration, \%detector);
    ...
    my $iv_ir=15.6;
    $par1 = $iv_ir/2; #InRad
    $par2 = ($iv_ir+$iv_side_tk)/2; #InRad+thick
    ...
    my $NchIV = 8; #MAX 8 otherwise overlasps with IV TOP/Bottom (9/10)
    $DeltaAng = 360./$NchIV;
    $DeltaToll = 1.;
   ...
   print_det(\%configuration, \%detector);
    ...
    my $w_ir=13.6;
    $par1 = $w_ir/2; #InRad
    $par2 = ($w_ir+$w_side_tk)/2; #InRad+thick
    ...
    $detector{"rotation"}    = "0*deg 0*deg 0*deg";
    $detector{"dimensions"}  = "0*cm $par1*cm $par3*cm $par4*deg $par5*deg";
    $detector{"material"}    = "BDX_Pb_shots";
    $detector{"material"}    = "BDX_W_shots";
    $detector{"material"}    = "G4_AIR";
    print_det(\%configuration, \%detector);
    ...
    # START PbWO4e crystals 400 0 15 TOP
    # START PbWO4e crystals 400 0 15 BOTTOM
    my $cr_PbWO4_ssx=1.5/2;
    my $cr_PbWO4_lsx=1.5/2;
    my $cr_PbWO4_ssy=1.5/2;
    my $cr_PbWO4_lsy=1.5/2;
    my $cr_PbWO4_lgt=20/2.;
    my $nline_1=5; #Closest line to CsI
    my $nline_2=3; #second line ...
    my $cr_PbWO4_airgap=0.05;# 1mm air between CsI and PbWOgap
    my $mlc1=($nline_1-1)/2*2*($cr_PbWO4_lsx+$cr_PbWO4_airgap); #most lateral crystal center position
    my $mlc2=($nline_2-1)/2*2*($cr_PbWO4_lsx+$cr_PbWO4_airgap); #most lateral crystal center position

    my $iCrystalIndex=-1;
    my $IndxAss=0;
    
    for(my $ir=0; $ir<2; $ir++) # Top BOTTOM
    {
        for(my $it=0; $it<2; $it++) # symmetric wrt CsI
        {
            for(my $im=0; $im<($nline_1); $im++)
            {
            $iCrystalIndex=$iCrystalIndex+1; #0-16
            my $imm=2*$ir-1;
            my $itt=2*$it-1;
            my $irr=2*$ir-1;
                #print 'Crystal indexing L1',"\n";
                #   print 'Index imm in itt : ',$iCrystalIndex,'  ',$im,' ',$itt,' ',$irr,"\n";
                $IndxAss=($im+1)*$itt+0;
            %detector = init_det();
            $detector{"name"}        = "PbWO4_L1"."_"."$IndxAss"."_"."$irr";
            $detector{"mother"}      = "mutest_W_air";
            $detector{"description"} = "PbWO4 crystal L1 "."_"."$IndxAss"."_"."$irr";
            $detector{"color"}       = "2d50ff";
            $detector{"style"}       = 0;
            $detector{"visible"}     = 1;
            $detector{"type"}        = "Trd";
                $X = $mlc1-$im*2*($cr_PbWO4_lsx+$cr_PbWO4_airgap);
                #print 'X: ',$X,' im: ',$im,"\n";
                $Y = $itt*($ar_wr_cr_lsy+$cr_PbWO4_lsy+$cr_PbWO4_airgap);
                # print 'Y: ',$Y,' itt: ',$itt,"\n";
                $Z = ($cr_PbWO4_lgt+$cr_PbWO4_airgap/2)*$irr;
                #   $X = ($nline_1*($cr_PbWO4_lsx+$cr_PbWO4_airgap))-2*($im+0.5)*($cr_PbWO4_lsx+$cr_PbWO4_airgap);
                #print 'Z: ',$Z,' irr: ',$irr,"\n";
                #$Y = $itt*($ar_wr_cr_lsy+$cr_PbWO4_lsy+$cr_PbWO4_airgap);
                # $Z = ($cr_PbWO4_lgt+$cr_PbWO4_airgap)*$imm;
            ...
            $detector{"sensitivity"} = "crs";
            $detector{"hit_type"}    = "crs";
            $detector{"identifiers"} = "sector manual 400 xch manual $IndxAss ych manual $irr";
            print_det(\%configuration, \%detector);
            }
        }
    }
    ...
    for(my $ir=0; $ir<2; $ir++) # Top BOTTOM
    {
        for(my $it=0; $it<2; $it++) # symmetric wrt CsI
        {
            for(my $im=0; $im<($nline_2); $im++)
            {
                $iCrystalIndex=$iCrystalIndex+1; #0-16
                my $imm=2*$ir-1;
                my $itt=2*$it-1;
                my $irr=2*$ir-1;
                #print 'Crystal indexing L2',"\n";
                #print 'Index imm in itt : ',$iCrystalIndex,'  ',$im,' ',$itt,' ',$irr,"\n";
                $IndxAss=($im+1)*$itt+$nline_1*abs(2*$it-1)/(2*$it-1);
                %detector = init_det();
                $detector{"name"}        = "PbWO4_L2"."_"."$IndxAss"."_"."$irr";
                $detector{"mother"}      = "mutest_W_air";
                $detector{"description"} = "PbWO4 crystal L2 "."$IndxAss"."_"."$irr";
                $detector{"color"}       = "2d50ff";
                $detector{"style"}       = 0;
                $detector{"visible"}     = 1;
                $detector{"type"}        = "Trd";
                $X = $mlc2-$im*2*($cr_PbWO4_lsx+$cr_PbWO4_airgap);
                #print 'X: ',$X,' im: ',$im,"\n";
                $Y = $itt*($ar_wr_cr_lsy+$cr_PbWO4_lsy+$cr_PbWO4_airgap+2*$cr_PbWO4_lsy+$cr_PbWO4_airgap);
                #print 'Y: ',$Y,' itt: ',$itt,"\n";
                $Z = ($cr_PbWO4_lgt+$cr_PbWO4_airgap/2)*$irr;
                #   $X = ($nline_1*($cr_PbWO4_lsx+$cr_PbWO4_airgap))-2*($im+0.5)*($cr_PbWO4_lsx+$cr_PbWO4_airgap);
                #print 'Z: ',$Z,' irr: ',$irr,"\n";
